...
export function MyWebLaneLanding() {
    ...
    const runScripts = () => {
      ...
      function scramble(target: string) {
        ...
        interval = setInterval(() => {
          ...
          if(iter >= target.length && interval) clearInterval(interval);
          iter += 1.5;
        }, 32);
      }
      ...
      if (window.gsap && window.ScrollTrigger) {
        ...
        document.querySelectorAll('.odometer').forEach((odo) => {
          const odoEl = odo as HTMLElement;
          ...
          digits.forEach((d) => {
            const digit = document.createElement('div'); digit.className = 'odo-digit';
            ...
            digit.appendChild(strip); odoEl.appendChild(digit);
          });
          
          if(suffix) {
            const s = document.createElement('span'); s.className = 'odo-suffix'; s.textContent = suffix; odoEl.appendChild(s);
          }
          ...
          window.ScrollTrigger.create({
            ...
            onEnter: () => {
              ...
              strips.forEach((strip, i) => {
                ...
                const h = (stripEl.children[0] as HTMLElement).offsetHeight || 60;
      ...
      if (widgetDemoBtn) {
        widgetDemoBtn.addEventListener('click', function(this: HTMLElement) {
          demo = !demo;
          ...
          fills.forEach((f) => {
            ...
            if (el) {
              ...
              if (el.nextElementSibling) el.nextElementSibling.textContent = f.v.toString();
            }
          });
          ...
          if (widgetTitle) widgetTitle.textContent = demo ? 'SYNTHIA Score · After' : 'SYNTHIA Score · Before';
  ...
  return (
    <div ref={containerRef} className="mwl-container" data-theme="dark">
