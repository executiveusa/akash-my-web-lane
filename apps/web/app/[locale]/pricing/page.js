...
const generateMetadata = async ({ params, }) => {
    return (0, metadata_1.createMetadata)({
        title: "Pricing",
        description: "Simple, transparent pricing for every market. India, US, and Mexico.",
    });
};
...
const Pricing = async ({ params }) => {
    ...
    const plans = marketPlans?.plans ?? [];
    const currency = locale === "hi" ? "₹" : locale === "es" ? "MXN " : "$";
    const guarantee = locale === "hi"
    ...
    return (<main className="min-h-screen bg-[#0a0a0a] text-[#f2ece0]">
      {/* Header */}
      ...
      <div className="container mx-auto px-4 py-20">
        <div className={`mx-auto grid max-w-5xl grid-cols-1 gap-6 ${plans.length === 3
            ? "md:grid-cols-3"
            : plans.length === 2
                ? "md:grid-cols-2"
                : "md:grid-cols-1"}`}>
          {plans.map((plan) => {
            ...
            return (<div key={plan.name} className={`relative flex flex-col rounded-2xl border p-8 ${isFeatured
                    ? "border-[#c9a84c]/50 bg-[#c9a84c]/5"
                    : "border-[#f2ece0]/10 bg-[#f2ece0]/3"}`}>
                {plan.badge && (<div className="absolute -top-3 left-1/2 -translate-x-1/2 rounded-full bg-[#c9a84c] px-4 py-1 font-mono text-xs font-bold text-[#0a0a0a] uppercase tracking-widest">
                    {plan.badge}
                  </div>)}
                ...
                <ul className="mb-8 flex-1 space-y-3">
                  {plan.features.map((feature) => (<li key={feature} className="flex items-start gap-2 text-sm text-[#f2ece0]/80">
                      <lucide_react_1.Check className="mt-0.5 h-4 w-4 shrink-0 text-[#4ade80]"/>
                      {feature}
                    </li>))}
                </ul>

                <button_1.Button asChild className={isFeatured
                    ? "w-full bg-[#c9a84c] font-bold text-[#0a0a0a] hover:bg-[#e2bf6a]"
                    : "w-full border-[#f2ece0]/20 text-[#f2ece0] hover:bg-[#f2ece0]/5"} size="lg" variant={isFeatured ? "default" : "outline"}>
                  <link_1.default href={env_1.env.NEXT_PUBLIC_APP_URL}>
                    {plan.cta} <lucide_react_1.MoveRight className="ml-2 h-4 w-4"/>
                  </link_1.default>
                </button_1.Button>
              </div>);
        })}
        ...
        <div className="mt-12 text-center">
          ...
          <button_1.Button asChild className="mt-3 border-[#f2ece0]/20 text-[#f2ece0] hover:bg-[#f2ece0]/5" variant="outline">
            ...
            : locale === "es"
                ? "Habla con nosotros"
                : "Talk to us"}{" "}
              <lucide_react_1.PhoneCall className="ml-2 h-4 w-4"/>
            </link_1.default>
          </button_1.Button>
        </div>
      </div>
