...
export async function generateDailyReports() {
    ...
    for (const job of jobs) {
        const report = {
            ...
            status: job.status,
            duration_seconds: Math.round((job.updatedAt.getTime() - job.createdAt.getTime()) / 1000),
        ...
        await fs.writeFile(path.join(reportDir, `job-${job.id}.json`), JSON.stringify(report, null, 2));
    }
    // Generate summary
