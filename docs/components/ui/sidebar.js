...
function SidebarProvider({ defaultOpen = true, open: openProp, onOpenChange: setOpenProp, className, style, children, ...props }) {
    ...
    return (<SidebarContext.Provider value={contextValue}>
      <tooltip_1.TooltipProvider delayDuration={0}>
        <div className={(0, utils_1.cn)("group/sidebar-wrapper flex min-h-svh w-full has-data-[variant=inset]:bg-sidebar", className)} data-slot="sidebar-wrapper" style={{
...
function SidebarTrigger({ className, onClick, ...props }) {
    ...
    return (<button_1.Button className={(0, utils_1.cn)("size-7", className)} data-sidebar="trigger" data-slot="sidebar-trigger" onClick={(event) => {
            onClick?.(event);
            toggleSidebar();
...
function SidebarMenuButton({ asChild = false, isActive = false, variant = "default", size = "default", tooltip, className, ...props }) {
    ...
    const button = (<Comp className={(0, utils_1.cn)(sidebarMenuButtonVariants({ variant, size }), className)} data-active={isActive} data-sidebar="menu-button" data-size={size} data-slot="sidebar-menu-button" {...props}/>);
    if (!tooltip) {
        return button;
    }
