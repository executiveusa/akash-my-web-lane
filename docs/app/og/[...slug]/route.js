...
const GET = async (_request, context) => {
    ...
    const page = source_1.source.getPage(slug.slice(0, -1));
    if (!page) {
        return new Response("Not found", { status: 404 });
    }
    const { title, description } = page.data;
    const text = `next-forge ${title} ${description}`;
    return new og_1.ImageResponse(<div tw="bg-neutral-100 relative flex flex-col justify-between w-full h-full">
      <div style={{
            backgroundSize: "48px 48px",
            backgroundImage: "linear-gradient(to right, #BBB 1px, transparent 1px), linear-gradient(to bottom, #BBB 1px, transparent 1px)",
      ...
      <div tw="flex top-12 left-12 z-10">
        ...
        <svg fill="none" height="48" viewBox="0 0 235 203" xmlns="http://www.w3.org/2000/svg">
          <path d="M117.082 0L234.164 202.794H0L117.082 0Z" fill="currentColor"/>
        </svg>
      </div>
      <div style={{ fontFamily: "Geist" }} tw="flex flex-col bottom-0 left-0 right-0 relative z-10 p-12">
        <p tw="text-lg m-0 text-black/80 font-semibold">next-forge</p>
        <h1 style={{ fontFamily: "Geist Semibold" }} tw="my-4 text-6xl font-bold text-black">
          {page.data.title}
        </h1>
